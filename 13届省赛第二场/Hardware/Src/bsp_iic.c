...
{
	IIC_Start();			//开始
	IIC_Send_Byte(0xa0);	//器件地址为写地址操作 24C02的芯片地址为1010 (A2 A1 A0)(R=1,W=0）0xA0为写
	IIC_Wait_Ack();		
	
	IIC_Send_Byte(ucAddr);//写地址
	IIC_Wait_Ack();
	
	while(ucNum--)	//写ucNum个数据
	{
		IIC_Send_Byte(*pucBuf++); //将此地址*pucBuf取值后写入
	 	IIC_Wait_Ack();
	}
	IIC_Stop();			//停止
	IIC_Delay();
...
void i2c_24c02_read(unsigned char *pucBuf , unsigned char ucAddr , unsigned char ucNum)
{
	...
	IIC_Start();			//再次开始
	IIC_Send_Byte(0xa1);	//器件地址为读地址操作 24C02的芯片地址为1010 (A2 A1 A0)(R=1,W=0）0xA1为读
	IIC_Wait_Ack();		
	
	while(ucNum--)	//读ucNum个数据
	{
		*pucBuf++ =IIC_Read_Byte(0); //将数据读入
	 	if(ucNum)
	 		IIC_Ack();
	 	else
	 		IIC_NAck();
